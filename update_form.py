...
old_handle_purchase = """  const handlePurchase = async () => {
...
new_handle_purchase = """  const handlePurchase = async () => {
...
old_form_start = """          {/* Form Steps */}
          <div className="space-y-8 w-full">"""

new_form_start = """          {/* Big Fillings Square Board */}
...
# Just do a generic replace for all three nested cards to divs
content = re.sub(r'<Card className="border-border/50 bg-card/30 backdrop-blur-sm overflow-hidden glass-card">', r'<div className="space-y-4">', content)
content = re.sub(r'<CardHeader className="bg-primary/5 border-b border-white/5">', r'<div className="mb-4">', content)
content = re.sub(r'</CardHeader>', r'</div>', content)
content = re.sub(r'<CardContent className="pt-6">', r'<div>', content)
content = re.sub(r'</CardContent>\s*</Card>', r'</div>\n              </div>', content)
...
old_integrated_purchase = """                  {/* Integrated Purchase Action */}
                  <div className="pt-6 border-t border-border/50">"""

new_integrated_purchase = """                  {/* Contact Information */}
...
old_end_form = """            </motion.div>
          </div>
        </div>"""

new_end_form = """            </motion.div>
          </div>
          </Card>
        </div>"""
...
# 4. Remove Dialogs
user_info_start = content.find('{/* User Info Dialog */}')
footer_start = content.find('{/* Footer */}')

if user_info_start != -1 and footer_start != -1:
    content = content[:user_info_start] + content[footer_start:]
